...
import { FileType_Format, FieldType, type FileType } from "$lib/protos/config_pb";
...
const fieldLocationValues: [FileType_Format | null, boolean | null] = [null, null];

$: {
  let format = filetype.format;
  let hasHeader = filetype.hasHeader;

  if (format !== fieldLocationValues[0] || hasHeader !== fieldLocationValues[1]) {
    ...
    <Grid>
      <Row>
        ...
        <Column>
          ...
          <Checkbox labelText="Sensitve Field" bind:checked={fieldtype.isSensitive} />
      ...
      <Row>
        ...
        <Column>
          {#if fieldtype.fieldLocation.case === 'headerColumn' }
            <TextInput labelText="Header Column Name" helperText="adfjadf jasf" bind:value={fieldtype.fieldLocation.value} />
          {:else}
      ...
      <Row>
        <Column>
          ...
          <FunctionMultiEditor bind:functionConfigs={fieldtype.validations}
