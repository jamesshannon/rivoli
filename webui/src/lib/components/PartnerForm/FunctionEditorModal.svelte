...
const items = Array.from(functionsMap.values())
    .filter((f) => f.type === functionType)
    .map((f) =>  ({id: f.id, text: f.name, description: f.description}));
</script>


<Modal on:open={openHandler} bind:open={open} modalHeading="Edit Function"
    size="lg" preventCloseOnClickOutside shouldSubmitOnEnter={false}
    primaryButtonText="Save" on:click:button--primary={saveChanges}
    secondaryButtonText="Cancel" on:click:button--secondary={() => (open = false)}>
  <div class="local">
  <Grid>
    <Row>
      <Column>
