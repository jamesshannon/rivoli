...
import { type Function, Function_DataType, Function_FunctionType } from "$lib/protos/functions_pb";
...
function saveChanges(evt) {
  ...
  const existingIdx = functionConfigs.findIndex((fc) => fc.id === funcConfig.id);
  if (existingIdx >= 0) {
...
<div>
  <ol>
    {#each functionConfigs as functionConfig (functionConfig.id)}
      ...
      <div class="function">{functionsMap.get(functionConfig.functionId)?.name}({functionConfig.parameters.join(', ')})</div>
      <Button size="small" kind="tertiary" on:click={() => editFunction(functionConfig.id)}
          iconDescription="Edit" icon={Edit} />
      <Button size="small" kind="tertiary" on:click={() => deleteFunction(functionConfig.id)}
          iconDescription="Remove" icon={Delete} />
    </li>
    {/each}
  </ol>
...
<FunctionEditorModal on:save={saveChanges} bind:open={modalOpen}
    functionConfig={tmpFuncConfig} {functionType}/>
