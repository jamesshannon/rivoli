...
function addNewFieldType() {
  recordtype.fieldTypes.push(new FieldType({active: true}));
...
<Grid>
  <Row>
    <Column><TextInput labelText="Record Type Name" bind:value={recordtype.name} /></Column>
  </Row>
  <Row>
    <Column>
  ...
  <FormGroup legendText="Processing Functions">
    <Row>
      <Column>
        ...
        <FunctionMultiEditor bind:functionConfigs={recordtype.validations}
    ...
    <Row>
      <Column>
        <FunctionEditor labelText="Upload Function"
            bind:functionConfig={recordtype.upload}
            functionType={Function_FunctionType.RECORD_UPLOAD} />
      </Column>
      <Column></Column>
    </Row>
  </FormGroup>
  ...
  <h3>Fields</h3>
  <Button kind="secondary" iconDescription="Add New Field Type" icon={Add}
  ...
  <Accordion align="start">
    {#each recordtype.fieldTypes as fieldtype}
      <AccordionItem open={false}>
        ...
        <FieldTypeSubForm bind:fieldtype={fieldtype} {filetype}  />
      </AccordionItem>
    {/each}
  </Accordion>
</Grid>
