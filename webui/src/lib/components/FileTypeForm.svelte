<script lang="ts">
	...
	let formatDropdownItems = [{ id: FileType_Format.FLAT_FILE_DELIMITED, text: 'Delimited' }];
		...
		<Grid>
			...
			<Row>
				...
				<Column>
					...
					<Checkbox
						labelText="Require Review Before Uploading"
			...
			<FormGroup legendText="File Format">
				...
				{#if filetype.format === FileType_Format.FLAT_FILE_DELIMITED}
					<Row>
						<Column>
							<Checkbox labelText="File has header" bind:checked={filetype.hasHeader} />
						</Column>
						<Column>
							<TextInput labelText="Column Separator" bind:value={filetype.delimitedSeparator} />
						</Column>
					</Row>
				{/if}
			</FormGroup>
			...
			<FormGroup legendText="Filename Filtering & Parsing">
				<Row>
					Filenames are detected and copied based on their filename. Additionally, you can parse the
					filename (using regexp patterns) to calculate file-level values, including parsing the
					file date from the filename and arbitary tags.
				</Row>
				<Row>
					<Column>
				...
				<Row>
					...
					<Column>
						<TextInput labelText="Filename Date Regexp" bind:value={filetype.filenameDateRegexp} />
					</Column>
					<Column>
						<TextInput labelText="Filename Date Format" bind:value={filetype.filenameDateFormat} />
					</Column>
				</Row>
				<Row>
					<Column>
			...
			<FormGroup legendText="Upload Batching">
				...
				<Row>
					<Column>
						<NumberInput label="Upload Batch Size" bind:value={filetype.uploadBatchSize} />
					</Column>
					<Column>
						<TextInput
