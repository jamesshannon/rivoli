...
const functionTypeDropdownItems = [
  {'id': Function_FunctionType.FIELD_VALIDATION,
  text: 'Field - Modify or Validate',
},
  {'id': Function_FunctionType.RECORD_VALIDATION,
  text: 'Record - Modify or Validate'},
]
...
console.log(func.functionStatement);
...
<Form on:submit={submitHandler}>
  <Grid>
    <Row>
      <Column><TextInput labelText="Function Name" disabled={readonly} bind:value={func.name} /></Column>
      <Column><Checkbox labelText="Active" disabled={readonly} bind:checked={func.active} /></Column>
    </Row>
    <Row>
      <Column>
    ...
    <Row>
      <Column>
        <Dropdown titleText="Function Type" disabled={readonly} bind:selectedId={func.type}
            items={functionTypeDropdownItems}/>
      </Column>
      <Column>
        <Dropdown titleText="Function Code or Pointer" disabled={readonly}
    ...
    <div class="statements">
      {#if func.functionStatement.case == 'pythonFunction'}
        <Row>
          <Column><TextInput labelText="Python Function" disabled={readonly} bind:value={func.functionStatement.value} /></Column>
        </Row>
      {:else if func.functionStatement.case == 'sqlCode'}
        <Row>
          <Column><TextArea labelText="SQL Statement" disabled={readonly} bind:value={func.functionStatement.value} /></Column>
        </Row>
      {/if}
    </div>
  </Grid>
...
<style>
.statements :global(input[type=text]), .statements :global(textarea) {
