<script lang="ts">
	...
	async function retrieveRecords(requestData, callback, settings) {
		console.log($page.route, $page.url.pathname, requestData, settings);
		...
		const docs: Array<Record> = await resp.json();
		...
		callback({
			...
			recordsFiltered: file.stats?.loadedRecordsSuccess
		});
	}
		...
		if (data.rawColumns) {
			lines.push(
				'<div><h5>Raw Columns</h5>' +
					data.rawColumns.map((str: string) => `<span>${str}</span>`).join(',') +
					'</div>'
			);
		}
		if (data.parsedFields) {
			lines.push(
				'<div><h5>Parsed Fields</h5>' + JSON.stringify(data.parsedFields, null, 2) + '</div>'
			);
		}
		if (data.validatedFields) {
			lines.push(
				'<div><h5>Validated Fields</h5>' + JSON.stringify(data.validatedFields, null, 2) + '</div>'
			);
		}
