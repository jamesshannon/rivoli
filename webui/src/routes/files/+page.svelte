...
function renderDate(data, type: string, row) {
  if (type === 'display') {
    return (new Date(row['created'] * 1000)).toLocaleString([], {dateStyle: 'short', timeStyle: 'short'});
  }
...
function renderRecordCount(data, type: String, row) {
  if (type === 'display') {
    return (row.stats?.totalRows ? row.stats.totalRows
...
function renderFileTypeName(data, type: string, row) {
  return `<a href="/partners/${row.partnerId}/filetypes/${row.fileTypeId}">${filetypeMap.get(row.fileTypeId).name}</a>`;
}
