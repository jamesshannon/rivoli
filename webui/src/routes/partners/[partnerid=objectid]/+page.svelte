<script lang="ts">
    ...
    const respbody = await (await fetch('/partners',
        {method: 'POST', body: partner.toJsonString()})).json();
...
<div id="notification">
  <InlineNotification kind="success" title="Success:" subtitle="Partner record saved" timeout={3000}/>
</div>

<PartnerForm bind:partner={partner} on:submit={submitHandler}/>
...
<DataTable headers={fileTypeHeader} rows={fileTypeRows}>
	<Toolbar>
        ...
        shouldFilterRows
      />
			<Link href="/partners/{partner.id}/filetypes/new"><Button kind="secondary" iconDescription="Add New File Type" icon={Add} /></Link>
		</ToolbarContent>
	</Toolbar>
